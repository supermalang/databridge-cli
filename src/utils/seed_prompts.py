...
_TEMPLATE_INFERENCE: ChatMessages = [
    {"role": "system", "content": (
        ...
        "respect its column requirements; note that bullet_list is not a real chart or "
        "graph, it just lists one column's raw values as bullet points, and should be "
        "used when there is no categorical column instead of table; for indicators, "
        "choose a stat from the indicator list. Respond with valid JSON only — no "
        "fences, no commentary."
    )},
    {"role": "user", "content": (
