...
For every baseline PNG under `frontend/tests/e2e/**-snapshots/` or
`frontend/tests/storybook/**-snapshots/` that differs from `develop` (committed or
uncommitted), look it up in `visual-approvals.json` (repo root) by its id (path relative to
`frontend/tests/`, POSIX separators) and classify:
...
A **human** runs the Tier 3 local review app (`node frontend/scripts/visual-review-app/server.mjs`
