...
4. **Visual (UI tasks)** — impeccable `audit`/`critique` findings on the changed UI are
   resolved (no open P0/P1), and a baseline exists for each of the three viewports (mobile
   390×844, tablet 820×1180, desktop 1440×900). "Exists" is not enough — dispatch
   `.claude/agents/visual-review.md` (or run `/visual-review`) and require its verdict to be
   `clear` for this task's baselines specifically; any of its `pending` or `rejected` entries
   naming this task's baseline ids is a FAIL (not a silent pass), even if the PNG is committed.
   For `N/A` E2E, confirm the task genuinely has no UI surface.
