...
> **Archived historical record.** Several of this audit's top findings have since shipped —
> the logframe / results chain (`framework:`, `{{ logframe }}`), baseline+target tracking on
> indicators (`pct_achievement`), the data-quality framework (`{{ data_quality }}`, completeness
> / outlier / duplicate rates), multi-period tracking (`periods:`), and indicator `direction`.
> The still-open gaps live in [docs/ROADMAP.md](../ROADMAP.md) under **M&E capabilities**. Kept
> here unedited for provenance; do not treat its "critical gaps" as current.
