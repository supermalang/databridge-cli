...
/**
 * MNT-25 removes bullet_list as a selectable Chart type in this editor (it becomes its
 * own first-class "Lists" section on the Composition page — see
 * `composition-bullet-list.spec.ts`). The former "MNT-21 — bullet_list preview renders
 * text" block (which exercised `setChartType(page, 'bullet_list')` through this modal's
 * Chart type dropdown) is retired here: MNT-25's AC1 requires `bullet_list` to no longer
 * appear in that dropdown at all, so driving it through this modal is no longer a valid
 * user flow. The regression coverage for "an ordinary image-based chart preview still
 * works" is retained implicitly by every other test in this file (they all exercise the
 * default seeded histogram chart).
 */
